...
@interface GTMOAuthTest : SenTestCase
@end

@implementation GTMOAuthTest
...
- (void)testParameterEncoding {
  ...
  for (NSString *key in dict) {
    ...
    NSString *output = [GTMOAuthAuthentication encodedOAuthParameterForString:input];
...
- (void)testRequestParameterNormalization {
  ...
  for (NSString *key in dict) {
    ...
    NSString *output = [GTMOAuthAuthentication normalizeQueryString:input];
...
- (void)testHMACSHA1Signing {
  ...
  result = [GTMOAuthAuthentication HMACSHA1HashForConsumerSecret:nil
                                                     tokenSecret:nil
                                                            body:nil];
  expected = @"5CoEcoq7XoKFjwYCieQvuzadeUA=";
  STAssertEqualObjects(result, expected, @"HMAC SHA-1 failed (cs)");

  result = [GTMOAuthAuthentication HMACSHA1HashForConsumerSecret:@"cs"
                                                     tokenSecret:nil
                                                            body:@"bs"];
...
- (void)testDictionaryWithResponseString {
  ...
  // test empty string
  testStr = nil;
  result = [GTMOAuthAuthentication dictionaryWithResponseString:testStr];
...
- (void)testPersistenceResponseString {
  ...
  // nothing set
  auth = [GTMOAuthAuthentication authForInstalledApp];
